...
<div class="card">
    <div class="card-header card-header-info flex-align">
        ...
        <a href="<?= route_to('admin_user_seeker') ?>" data-method="get"
           data-prompt="Những thay đổi đều chưa được lưu. Bạn có muốn tiếp tục?" class="btn btn-round">Huỷ</a>
        <a href="<?= route_to('admin_user_seeker_delete', $model->getPrimaryKey()) ?>" data-method="post"
           data-prompt="Bạn có chắc sẽ xoá đi mục này?" class="btn btn-danger btn-round">Xoá</a>
    </div>
    <div class="card-body">
        <form action="<?= route_to('admin_user_seeker_update', $model->getPrimaryKey()) ?>" method="post"
