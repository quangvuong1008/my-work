<div class="tablehs-01-box">
    <div class="title-nor d-mb" hidden id="countMessReceive_<?= $tab ?>"> (<?= $countMess ?>)</div>
    ...
    <?php if ($messData && !empty($messData)) : ?>
        <?php foreach ($messData as $key => $md) : ?>
            <div class="tablehs-01-row message-page__item row align-items-center mx-1 mx-md-0">
                <div class="tablehs-01-row-td d-flex align-items-center message-page__col-information" onclick="showModalMess('<?= $md->id ?>', '<?= $md->status_mess ?>', '<?= $md->id_candidate ?>', '<?= $md->slug ?>', '<?= $md->id_send ?>', '<?= $md->id_receive ?>')">
                    ...
                    <div class="col-12 px-1 px-md-3 col-md-10 message-page__col-information-title" style="padding-left: 1rem!important; padding-right: 1rem!important;">
                        <a class="d-flex-mess align-items-center">
                            <?php if ($md->status_mess == '1') : ?>
                                <div class="message-active"></div>
                            <?php endif; ?>
                            <span class="text-ellipsis"><?= $md->header ?></span>
                        </a>
                        <div class="info-user mt-1"><i class="far fa-user" style="font-size: 13px; margin-right: 3px;"></i><span class="font500 align-middle">Gửi bởi: <?= $md->full_name ?></span></div>
                    </div>
                </div>
                <div class=" tablehs-01-row-td message-page__col-created pl-0-mb" onclick="showModalMess('<?= $md->id ?>', '<?= $md->status_mess ?>')">
                ...
                <div class="tablehs-01-row-td message-page__col-action align-items-center d-none d-md-flex hidden-xs">
                    <button class="btn btn-info mr-2" type="button" onclick="showModalMess('<?= $md->id ?>', '<?= $md->status_mess ?>')"><i class="far fa-eye"></i>
                    </button>
