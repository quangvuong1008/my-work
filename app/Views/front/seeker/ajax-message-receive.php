    ...
    <?php if ($messData && !empty($messData)) : ?>
        <?php foreach ($messData as $key => $md) : ?>
            <div class="tablehs-01-row message-page__item row align-items-center mx-1 mx-md-0">
                <div class="tablehs-01-row-td d-flex align-items-center message-page__col-information" onclick="showModalSeekerMess('<?= $md->id ?>', '<?= $md->status_mess ?>', '<?= $md->id_send ?>', '<?= $md->id_receive ?>')">
                ...
                <div class=" tablehs-01-row-td message-page__col-created pl-0-mb" onclick="showModalSeekerMess('<?= $md->id ?>', '<?= $md->status_mess ?>')">
                ...
                <div class="tablehs-01-row-td message-page__col-action align-items-center d-none d-md-flex hidden-xs">
                    <button class="btn btn-info mr-2" type="button" onclick="showModalSeekerMess('<?= $md->id ?>', '<?= $md->status_mess ?>')"><i class="far fa-eye"></i>
                    </button>
