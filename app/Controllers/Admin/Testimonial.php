...
class Testimonial extends BaseController
{
    ...
    public function index()
    {
        ...
        if (($keyword = $this->request->getGet('keyword')) !== null) {
            $model->like('full_name', $keyword)->orLike('intro', $keyword);
        }
    ...
    public function create()
    {
        ...
        if ($this->isPost() && $this->validate($model->getRules())) {
            try {
                ...
                return $this->response->redirect(route_to('admin_testimonial'));
            } catch (\Exception $ex) {
                SessionHelper::getInstance()->setFlash('ALERT', [
                    'type' => 'danger',
    ...
    {
        /** @var TestimonialModel $model */
        $model = (new TestimonialModel())->find($id);
    ...
    {
        /** @var TestimonialModel $model */
        if (!$this->isPost() || !($model = (new TestimonialModel())->find($id))) {
            return $this->renderError();
        }
